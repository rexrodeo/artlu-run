...
# ======================================================================
# PURCHASE LOOKUP API (for OpenClaw)
# ======================================================================

@app.route('/api/purchase-lookup', methods=['GET'])
def lookup_purchase():
    """
    Look up a purchase by email + race slug.
    OpenClaw uses this to get the purchase_id after receiving a Stripe webhook,
    since the purchase_id is generated by artlu.run's own webhook handler.

    Query params: email, race_slug
    Returns: {purchase_id, status} or 404
    """
    api_key = request.headers.get('X-API-Key', '')
    if api_key != os.getenv('OPENCLAW_API_KEY', 'dev-key'):
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    race_slug = request.args.get('race_slug', '')

    if not email or not race_slug:
        return jsonify({'error': 'email and race_slug required'}), 400

    race = get_race_by_slug(race_slug)
    if not race:
        return jsonify({'error': 'Race not found'}), 404

    purchase = get_purchase_for_race(email, race['id'])
    if not purchase:
        return jsonify({'error': 'Purchase not found', 'hint': 'Stripe webhook may not have fired yet. Retry in a few seconds.'}), 404

    return jsonify({
        'purchase_id': purchase['id'],
        'email': email,
        'race_slug': race_slug,
        'race_name': purchase['race_name'],
        'goal_time': purchase['goal_time'],
        'city': purchase['city'],
        'state': purchase['state'],
        'has_premium_data': bool(purchase['premium_data_json']),
        'report_ready': bool(purchase['report_ready']),
    })
